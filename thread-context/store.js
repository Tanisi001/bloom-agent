 ...
 * @property {Array<{role: string, parts: Array<{text: string}>}>} history
...
export class SessionStore {
   ...
   * @returns {Array<{role: string, parts: Array<{text: string}>}> | null}
   */
  ...
  setHistory(channelId, threadTs, history) {
    const key = `${channelId}:${threadTs}`;
    this._store.set(key, { history, timestamp: Date.now() });
    this._cleanup();
  }
